...
bool PLM(Arrays *u, int i, int j, int k) {
    ...
    if (nx > 1) { for (int n = 0; n < NWAVE; n++) { dw_x[n] = sgn(dwC_x[n])*fminl(fminl(2.0*fabsl(dwL_x[n]), 2.0*fabsl(dwR_x[n])), fabsl(dwC_x[n]));  if (null(dw_x[n])) dw_x[n] = 0.0; } }
    if (ny > 1) { for (int n = 0; n < NWAVE; n++) { dw_y[n] = sgn(dwC_y[n])*fminl(fminl(2.0*fabsl(dwL_y[n]), 2.0*fabsl(dwR_y[n])), fabsl(dwC_y[n]));  if (null(dw_y[n])) dw_y[n] = 0.0; } }
    if (nz > 1) { for (int n = 0; n < NWAVE; n++) { dw_z[n] = sgn(dwC_z[n])*fminl(fminl(2.0*fabsl(dwL_z[n]), 2.0*fabsl(dwR_z[n])), fabsl(dwC_z[n]));  if (null(dw_z[n])) dw_z[n] = 0.0; } }
