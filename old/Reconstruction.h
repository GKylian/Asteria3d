...
bool HDPLM(Arrays *us, int xi, int yi, Arrays *int_x, Arrays *int_y, ld dt, ld gx, ld gy, ld gz, ld rs) {
    ld dx = us->dx; ld dy = us->dy;

    //0.1) Transform the variables at (i-1, i, i+1) and (j-1, (j,) j+1)
    ld wi[8] = { 0 }; for (int i = 0; i < 5; i++) wi[i] = us->at(i, xi, yi);
    if (!toPrimitive(wi, wi)) cout << "First conversion to primitive failed." << endl;
    ld wj[8] = { 0 }; copy(wi, wi+8, wj); swap(wj[1], wj[2]);

    ld wip[8] = { 0 }; for (int i = 0; i < 5; i++) wip[i] = us->at(i, xi+1, yi);
    if (!toPrimitive(wip, wip)) cout << "First conversion to primitive failed." << endl;
    ld wim[8] = { 0 }; for (int i = 0; i < 5; i++) wim[i] = us->at(i, xi-1, yi);
    if (!toPrimitive(wim, wim)) cout << "First conversion to primitive failed." << endl;



    ld wjp[8] = { 0 }; for (int i = 0; i < 5; i++) wjp[i] = us->at(i, xi, yi+1);
    if (!toPrimitive(wjp, wjp)) cout << "First conversion to primitive failed." << endl; swap(wjp[1], wjp[2]);
    ld wjm[8] = { 0 }; for (int i = 0; i < 5; i++) wjm[i] = us->at(i, xi, yi-1);
    if (!toPrimitive(wjm, wjm)) cout << "First conversion to primitive failed." << endl; swap(wjm[1], wjm[2]);


    //0.2) Get eigensystems
    ld eigen_x[5] = { 0 }; ld Lx[5][5] = { 0 }; ld Rx[5][5] = { 0 };
    getEigenHD(wi, eigen_x, Lx, Rx);
    ld eigen_y[5] = { 0 }; ld Ly[5][5] = { 0 }; ld Ry[5][5] = { 0 };
    getEigenHD(wj, eigen_y, Ly, Ry);

    ld eigM_x = (ld)*max_element(eigen_x, eigen_x+5); ld eig0_x = (ld)*min_element(eigen_x, eigen_x+5);
    ld eigM_y = (ld)*max_element(eigen_y, eigen_y+5); ld eig0_y = (ld)*min_element(eigen_y, eigen_y+5);



    //1) Compute L/R, centered and van Leer differences.
    ld dwL_x[5] = { 0 }; ld dwR_x[5] = { 0 }; ld dwC_x[5] = { 0 }; ld dwG_x[5] = { 0 };
    ld dwL_y[5] = { 0 }; ld dwR_y[5] = { 0 }; ld dwC_y[5] = { 0 }; ld dwG_y[5] = { 0 };
    ...
    ld daL_x[5] = { 0 }; ld daR_x[5] = { 0 }; ld daC_x[5] = { 0 }; ld daG_x[5] = { 0 };
    ld daL_y[5] = { 0 }; ld daR_y[5] = { 0 }; ld daC_y[5] = { 0 }; ld daG_y[5] = { 0 };
    for (int n = 0; n < 5; n++)
        for (int m = 0; m < 5; m++)
        {
    ...
    ld da_x[5] = { 0 }; ld da_y[5] = { 0 };
    for (int n = 0; n < 5; n++)
    {
        /*ld lim1_x = fminl(fabsl(daL_x[n]), fabsl(daR_x[n])); ld lim2_x = fminl(fabsl(daC_x[n]), fabsl(daG_x[n]));
        ld lim1_y = fminl(fabsl(daL_y[n]), fabsl(daR_y[n])); ld lim2_y = fminl(fabsl(daC_y[n]), fabsl(daG_y[n]));
    ...
    ld wL_x[5] = { 0 }; ld wR_x[5] = { 0 };
    ld wL_y[5] = { 0 }; ld wR_y[5] = { 0 };
    for (int i = 0; i < 5; i++)
    {
    ...
    ld x = us->x0 + (xi-4.0)*dx; ld y = us->y0 + (yi-4.0)*dy;

    if (rs >= 0.0) {
        ld xm = x-dx/2.0; ld xp = x+dx/2.0;   ld ym = y-dy/2.0; ld yp = y+dy/2.0;

        //S = 0, -rh*dphidx, -rh*dphidy, 0, -rho*(vx*dphidx+vy*dphidy)
        ld dpdx_m = Phix(xm, y, rs); ld dpdx_p = Phix(xp, y, rs);
        ld dpdy_m = Phiy(x, ym, rs); ld dpdy_p = Phiy(x, yp, rs);
    ...
    ld uxL[8] = { wL_x[0], wL_x[1], wL_x[2], wL_x[3], wL_x[4], 0, 0, 0 }; ld uxR[8] = { wR_x[0], wR_x[1], wR_x[2], wR_x[3], wR_x[4], 0, 0, 0 };
    ld uyL[8] = { wL_y[0], wL_y[2], wL_y[1], wL_y[3], wL_y[4], 0, 0, 0 }; ld uyR[8] = { wR_y[0], wR_y[2], wR_y[1], wR_y[3], wR_y[4], 0, 0, 0 };
...
bool PLM(Arrays *us, int xi, int yi, Arrays *int_x, Arrays *int_y, ld dt, ld gx, ld gy, ld gz, bool CharTrac, ld rs) {
    ld dx = us->dx; ld dy = us->dy;
    ...
    ld wi[8] = { 0 }; for (int i = 0; i < 8; i++) wi[i] = us->at(i, xi, yi);
    ...
    ld wj[8] = { 0 }; copy(wi, wi+8, wj); swap(wj[1], wj[2]); swap(wj[5], wj[7]);

    ld wip[8] = { 0 }; for (int i = 0; i < 8; i++) wip[i] = us->at(i, xi+1, yi);
    ...
    ld wim[8] = { 0 }; for (int i = 0; i < 8; i++) wim[i] = us->at(i, xi-1, yi);
    ...
    ld wjp[8] = { 0 }; for (int i = 0; i < 8; i++) wjp[i] = us->at(i, xi, yi+1);
    ...
    ld wjm[8] = { 0 }; for (int i = 0; i < 8; i++) wjm[i] = us->at(i, xi, yi-1);
    ...
    ld eigen_x[7] = { 0 }; ld Lx[7][7] = { 0 }; ld Rx[7][7] = { 0 };
    getEigen(wi, eigen_x, Lx, Rx);
    ld eigen_y[7] = { 0 }; ld Ly[7][7] = { 0 }; ld Ry[7][7] = { 0 };
    getEigen(wj, eigen_y, Ly, Ry);

    ld eigM_x = (ld)*max_element(eigen_x, eigen_x+7); ld eig0_x = (ld)*min_element(eigen_x, eigen_x+7);
    ld eigM_y = (ld)*max_element(eigen_y, eigen_y+7); ld eig0_y = (ld)*min_element(eigen_y, eigen_y+7);
    ...
    ld dwL_x[7] = { 0 }; ld dwR_x[7] = { 0 }; ld dwC_x[7] = { 0 };
    ld dwL_y[7] = { 0 }; ld dwR_y[7] = { 0 }; ld dwC_y[7] = { 0 };
    ...
    //2) Project the differences along characteristics
    ld daL_x[7] = { 0 }; ld daR_x[7] = { 0 }; ld daC_x[7] = { 0 }; ld daG_x[7] = { 0 };
    ld daL_y[7] = { 0 }; ld daR_y[7] = { 0 }; ld daC_y[7] = { 0 }; ld daG_y[7] = { 0 };
    for (int n = 0; n < 7; n++)
    for (int m = 0; m < 7; m++)
    {
    ...
    ld wL_x[7] = { 0 }; ld wR_x[7] = { 0 };
    ld wL_y[7] = { 0 }; ld wR_y[7] = { 0 };
    for (int i = 0; i < 7; i++)
    {
        wL_x[i] = wi[i] + 0.5*dw_x[i];
        wR_x[i] = wi[i] - 0.5*dw_x[i];
    ...
    ld dBy = dt/2.0 * wi[2]*(us->at(5, xi+1, yi)-us->at(5, xi, yi))/dx;   ld dBx = dt/2.0 * wi[1]*(us->at(6, xi, yi+1)-us->at(6, xi, yi))/dy;
    ...
    if (rs >= 0.0) {
        ld xm = x-dx; ld xp = x+dx;   ld ym = y-dy; ld yp = y+dy;
    ...
    ld Bx_R = us->at(5, xi, yi); ld Bx_L = us->at(5, xi+1, yi);
    ld By_R = us->at(6, xi, yi); ld By_L = us->at(6, xi, yi+1);

    //x: (rho, vx, vy, vz, P, By, Bz, Bx);    y: (rho, vy, vx, vz, P, Bx, Bz, By)
    ld uxL[8] = { wL_x[0], wL_x[1], wL_x[2], wL_x[3], wL_x[4], Bx_L, wL_x[5], wL_x[6] }; ld uxR[8] = { wR_x[0], wR_x[1], wR_x[2], wR_x[3], wR_x[4], Bx_R, wR_x[5], wR_x[6] };
    ld uyL[8] = { wL_y[0], wL_y[2], wL_y[1], wL_y[3], wL_y[4], wL_y[5], By_L, wL_y[6] }; ld uyR[8] = { wR_y[0], wR_y[2], wR_y[1], wR_y[3], wR_y[4], wR_y[5], By_R, wR_y[6] };
...
bool PLM_Simple(Arrays *us, int xi, int yi, Arrays *int_x, Arrays *int_y, ld dt, ld gx, ld gy, ld gz, bool CharTrac) {
    ld dx = us->dx; ld dy = us->dy;
    ...
    // Add gravity-related source terms for M and E.
    ld x = us->x0 + (xi-4.0)*dx; ld y = us->y0 + (yi-4.0)*dy;
    ld S[5] = { 0, wi[0]*gx, wi[0]*gy, wi[0]*gz, wi[0]*(wi[1]*gx+wi[2]*gy+wi[3]*gz) };
...
bool PLMInterfaces_Prim(Arrays *us, int xi, int yi, Arrays *int_x, Arrays *int_y, ld dt) {
    //rho, Mx, My, Mz, E, Bx, By, Bz
    //No need to interpolate for Bx and By
    //cout << xi << ", " << yi << endl;
    ...
    ld wixp[8] = { 0 }; for (int i = 0; i < 8; i++) wixp[i] = us->at(i, xi+1, yi);
...
bool PPMInterfaces(Arrays *us, int xi, int yi, Arrays *int_x, Arrays *int_y) {
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        //minus,plus interfaces for x,y
        ld duxp = us->at(i, xi+1, yi)-us->at(i, xi, yi); ld duxm = us->at(i, xi, yi)-us->at(i, xi-1, yi);
        ld duyp = us->at(i, xi, yi+1)-us->at(i, xi, yi); ld duym = us->at(i, xi, yi)-us->at(i, xi, yi-1);
        ld k = 1.0/3.0;
...
bool PPMInterfaces_Prim(Arrays *us, int xi, int yi, Arrays *int_x, Arrays *int_y) {
    ...
    ld r_x[8] = { 0 }; ld r_y[8] = { 0 };
    ld wi[8] = { 0 }; for (int i = 0; i < 8; i++) wi[i] = us->at(i, xi, yi); toPrimitive(wi, wi);
    ld wip[8] = { 0 }; for (int i = 0; i < 8; i++) wip[i] = us->at(i, xi+1, yi); toPrimitive(wip, wip);
    ld wim[8] = { 0 }; for (int i = 0; i < 8; i++) wim[i] = us->at(i, xi-1, yi); toPrimitive(wim, wim);
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        ld dwxp = wim[i]-wi[i]; ld dwxm = wi[i]-wim[i];
        ld k = 1.0/3.0;
